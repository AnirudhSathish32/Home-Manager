...
def scan(store, source, inbox=False):
    job = store.create_job(source)
    Scanner(store, ScanLimits(stability_seconds=0)).run(job, source, inbox=inbox)
    return job
...
def test_external_copies_independent_and_manual_moves_preserve_originals(library):
    store, source = library
    original = source / "2026" / "09" / "bank.csv"
    original.write_bytes(b"date,amount\n2026-09-24,25.00\n")
    scan(store, source)
    doc = store.documents(source)["items"][0]
    copied = store.library.path(doc["managed_path"])
    blob = store.blob_path(doc["current_hash"])
    assert copied.read_bytes() == blob.read_bytes() == original.read_bytes()
    assert all((store.library.root / name).is_dir() for name in LIBRARY_FOLDERS)
    store.library_action(source, doc["id"], doc["current_hash"], "move", "Bank_Statements")
    moved = store.documents(source)["items"][0]
    assert moved["folder"] == "Bank_Statements"
    assert not copied.exists()
    assert store.library.path(moved["managed_path"]).read_bytes() == original.read_bytes()
    scan(store, source)
    assert store.documents(source)["items"][0]["managed_path"] == moved["managed_path"]
    store.library.path(moved["managed_path"]).write_bytes(b"user edited")
    assert blob.read_bytes() == original.read_bytes()
    with pytest.raises(ValueError, match="edited"):
        store.library_action(source, doc["id"], doc["current_hash"], "move", "Bills")
    assert store.library.path(moved["managed_path"]).read_bytes() == b"user edited"
    assert store.documents(source)["items"][0]["managed_status"] == "blocked"
...
def test_inbox_capture_before_move_and_recovery_after_move(library, monkeypatch):
    ...
    scan(store, store.library.inbox, inbox=True)
    doc = store.documents(source)["items"][0]
    ...
    with pytest.raises(SimulatedCrash):
        store.library_action(source, doc["id"], doc["current_hash"], "move", "Receipts")
    monkeypatch.setattr(store.library, "finish", finish)
    store.library.recover()
    organized = store.documents(source)["items"][0]
    ...
    store.library.recover()
    scan(store, store.library.inbox, inbox=True)
    assert store.documents(source)["items"][0]["source_status"] == "organized"
...
def test_capture_failure_keeps_inbox_file(library, monkeypatch):
    ...
    job = scan(store, store.library.inbox, inbox=True)
...
def test_changed_inbox_versions_and_readded_identical_file(library):
    ...
    scan(store, store.library.inbox, inbox=True)
    first = store.documents(source)["items"][0]
    path.write_bytes(b"replacement bytes")
    scan(store, store.library.inbox, inbox=True)
    second = store.documents(source)["items"][0]
    ...
    named = store.documents(source)["items"][0]["managed_path"]
    path.write_bytes(b"replacement bytes")
    scan(store, store.library.inbox, inbox=True)
    assert not path.exists()
    assert store.documents(source)["items"][0]["managed_path"] == named
    assert len(list((store.library.root / "Receipts").iterdir())) == 1
...
def test_restart_backfills_capture_before_organization_intent(tmp_path, monkeypatch):
    ...
    try:
        ...
        assert len(list((store.library.root / "Unfiled").iterdir())) == 1
    finally:
        store.close()
...
def test_recovery_never_truncates_a_published_file_renamed_by_user(library, monkeypatch):
    ...
    # Fail after the initial external copy is published but before its DB commit.
    class Crash(BaseException):
        pass
    finish = store.library.finish
    def crash(intent):
    ...
    with pytest.raises(Crash):
        scan(store, source)
    target = next((store.library.root / "Unfiled").iterdir())
    ...
    doc = store.documents(source)["items"][0]
    assert store.library.path(doc["managed_path"]).read_bytes() == original.read_bytes() == b"captured"
...
def test_collision_and_path_confinement(library):
    ...
    for name in ("a.csv", "b.csv"):
        (source / "2026" / "09" / name).write_bytes(content)
    scan(store, source)
    docs = store.documents(source)["items"]
    ...
    store.library_action(source, doc["id"], doc["current_hash"], "move", "Receipts")
    moved = next(item for item in store.documents(source)["items"] if item["id"] == doc["id"])
...
def test_inbox_analysis_files_supported_receipt_and_preserves_manual_override(tmp_path, local_model):
    ...
    try:
        ...
        filed = manager.store.documents(source)["items"][0]
        ...
        uncertain = manager.store.documents(source)["items"][0]
        assert uncertain["folder"] == "Receipts"
        manager.library_action(doc["id"], doc["current_hash"], "move", "Bills")
        manager.start_reasoning(doc["id"], parse_id, True); manager.future.result(timeout=15)
        assert manager.store.documents(source)["items"][0]["folder"] == "Bills"
    finally:
        manager.close()
