...
def test_pdf_preview_uses_authenticated_preserved_version(tmp_path):
    ...
    with TestClient(app, base_url=URL) as client:
        ...
        pdf = manager.store.library.inbox / "receipt.pdf"
        ...
        pdf.unlink()  # Preview must use the captured copy, not the Inbox file.
...
def test_config_scan_history_and_restart(tmp_path):
    ...
    config = {"managed_directory": str(tmp_path / "managed")}
    with TestClient(create_app(control, "test-session-token", limits=ScanLimits(stability_seconds=0)), base_url=URL, headers=AUTH) as client:
        assert client.post("/api/inbox-scans", json={}).status_code == 400
        assert client.put("/api/settings", json={**config, "source_directory": str(tmp_path)}).status_code == 422  # No longer a setting.
        assert client.put("/api/settings", json=config).status_code == 200
        (tmp_path / "managed" / "Library" / "Inbox" / "my.csv").write_bytes(b"date,total\n2026-09-01,123\n")
        assert client.post("/api/scans", json={}).status_code == 405  # The year/month source scan is gone.
        response = client.post("/api/inbox-scans", json={})
        ...
        assert client.get(f"/api/scans/{job}/events").json()[0]["relative_path"] == "my.csv"
...
def test_configuration_cannot_change_during_scan(tmp_path, monkeypatch):
    ...
    with TestClient(create_app(tmp_path / "control", "test-session-token"), base_url=URL, headers=AUTH) as client:
        ...
        try:
            assert client.post("/api/inbox-scans", json={}).status_code == 202
            assert entered.wait(timeout=2)
            assert client.put("/api/settings", json=config).status_code == 409
            assert client.post("/api/inbox-scans", json={}).status_code == 409
        finally:
            release.set()
